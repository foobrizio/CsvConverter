class CsvReader:
    ...
    def __init__(self):
        self.wind_speed_sum = 0             # OpenWeatherMap
        self.wind_force_sum = 0
        self.wind_dir_sum = 0               # OpenWeatherMap
        self.wind_dir_degrees_sum = 0       # OpenWeatherMap
        self.humidity_sum = 0               # sensors
        self.temperature_sum = 0            # sensors
        self.noise_sum = 0
        self.pm2_5_sum = 0                  # OpenWeatherMap
        self.pm10_sum = 0                   # OpenWeatherMap
        self.atm_sum = 0                    # sensors
        self.light_sum = 0                  # sensors
        self.light_hun_sum = 0              # sensors
        self.rain_sum = 0                   # OpenWeatherMap
        self.cont = 0

    def collect_row(self, row: list):
        self.wind_speed_sum += float(row[1])
        self.wind_force_sum += float(row[2])
        ...
        self.light_hun_sum += float(row[12])
        self.rain_sum += float(row[13])
        self.cont += 1

    def perform_average(self):
        result = [round(self.wind_speed_sum / self.cont, 1), round(self.wind_force_sum / self.cont, 1),
                  ...
                  round(self.light_sum / self.cont, 1), round(self.light_hun_sum / self.cont, 1),
