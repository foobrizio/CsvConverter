...
def create_new_file():
    my_new_file = path + "/final_dataset"
    if time_setting == 1:
        my_new_file += "_half.csv"
    elif time_setting == 0:
        my_new_file += "_quarter.csv"
    else:
        my_new_file += "_hour.csv"
    if os.path.exists(my_new_file):
...
def reduce(csv_file: str):
    ...
    with open(csv_file) as old_dataset:
        ...
        time_manager = TimeManager(time_setting=time_setting, first_timestamp=results.values[0][0].split(';')[0])
        try:
